...
example.propertypane.PropertyPaneActivity = Class.extend({
		...
		app.getBackend().getActivities($.proxy(function(activities){
		    ...
		    var templ= "<form class='form-horizontal'>"+
			 ...
			 $("#activityParameterMapping").handsontable({
                 ...
                 minSpareRows: 0,
 	             stretchH: 'last',
			     colHeaders: ["Input Parameter", "Assigned context variable"],
			     afterSelectionEnd: $.proxy(function(row, column){
			        if(column==1){
			            (new example.dialog.SelectContextVariableDialog($.proxy(function(varName){
			                var mapping =this.figure.getActivity().mapping;
			                app.executeCommand( new example.command.CommandSetJSON(this.figure, mapping[row], {contextVariable:varName}));
			            },this))).show();
			        }
			     },this),
			     columns: [
			       {data:"activityParameter"  , type:{renderer: activityReadOnlyRenderer}, readOnly:true },
			       {data:"contextVariable", type:{renderer: activityReadOnlyRenderer}    , readOnly:true }
			     ]
			   });
	...
	_onActivitySelect: function(activityId, initialCall){
	    ...
	    // activityDef
	        ...
	        app.executeCommand(new example.command.CommandSetActivityDef(this.figure,{activity:activityId, mapping:activityDef.parameters} ));
	    }
               
	    // rerender the mapping table
        $('#activityParameterMapping').handsontable('loadData',this.figure.getActivity().mapping);
        if(activityDef.parameters.length!=0){
